\begin{tikzpicture}
	...
	\begin{axis}[width = 12cm,
		...
		\addplot+[draw=graph1,fill=graph1!60!white] table [col sep=comma, x=nas, y expr=\thisrow{ratio}-1] {figures/evaluation/performance/results-A.dat};
		\addlegendentry{\small Input Size~A}
		\addplot+[draw=graph2,fill=graph2!60!white] table [col sep=comma, x=nas, y expr=\thisrow{ratio}-1] {figures/evaluation/performance/results-B.dat};
		\addlegendentry{\small Input Size~B}
		\addplot+[draw=graph3,fill=graph3!60!white] table [col sep=comma, x=nas, y expr=\thisrow{ratio}-1] {figures/evaluation/performance/results-C.dat};
